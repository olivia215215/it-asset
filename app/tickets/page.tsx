...
export default function TicketsPage() {
  ...
  const fetchTickets = async (p: number) => {
    ...
    setError("");
    try {
      const res = await apiFetch<PaginatedResponse<TicketItem>>("/api/tickets", {
  ...
  const statusBadge = (status: string) => {
    const map: Record<string, string> = {
      SUBMITTED: "bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-400",
    ...
    const label: Record<string, string> = {
      SUBMITTED: "已提交",
      PENDING_MANAGER_1: "待主管审批",
      PENDING_MANAGER_2: "待上级审批",
      PENDING_IT: "待IT处理",
      APPROVED: "已通过",
      REJECTED: "已驳回",
      IN_PROGRESS: "处理中",
